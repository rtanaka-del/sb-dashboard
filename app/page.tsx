...
type SheetConfig = { id: string; name: string; description: string; tabs: string[]; status: 'idle' | 'loading' | 'success' | 'error'; lastUpdated?: string; };
...
// ==========================================
// 2. 定数・初期モックデータ (Constants & Mock Data)
// ==========================================
...
const MOCK_NEW_DATA: NewSalesRecord[] = [{ segment: 'Enterprise', budget: 5000, target: 5500, actual: 4800, last_year: 4000, count: 5, win_rate: 35, lead_time: 120, unit_price: 840, id_price: 2000, duration: 12 }, { segment: 'Mid', budget: 3000, target: 3300, actual: 3200, last_year: 2800, count: 12, win_rate: 45, lead_time: 60, unit_price: 291, id_price: 1500, duration: 12 }, { segment: 'Small', budget: 1500, target: 1800, actual: 1600, last_year: 1200, count: 30, win_rate: 60, lead_time: 30, unit_price: 60, id_price: 1200, duration: 12 }];
const MOCK_EXISTING_DATA: ExistingSalesRecord[] = [{ segment: 'Enterprise', sales: 12134547, nrr: 60.1, renewal: 92.3, id_growth: 63.9 }, { segment: 'Mid', sales: 6942000, nrr: 61.3, renewal: 60.0, id_growth: 68.2 }, { segment: 'Small', sales: 690000, nrr: 83.6, renewal: 100.0, id_growth: 92.0 }];
const MOCK_LIST_DATA: ListRecord[] = [
  ...
  { category: 'NotRenewed', date: '', name: 'Zマート', segment: 'Small', amount: 100, count: 5, owner: '鈴木', status: '', memo: '価格面で折り合わず', extra1: '' },
  { category: 'Fluctuation', date: '', name: 'Xホールディングス', segment: 'Enterprise', amount: 2500, count: 120, owner: '', status: '', memo: '部署拡大', extra1: '+25%' },
  { category: 'HighValue', date: '', name: 'イノベーション', segment: 'Enterprise', amount: 4500, count: 300, owner: '', status: '', memo: '2023/04/01開始', extra1: '' },
...
const MOCK_OKR_DATA: OkrRecord[] = [{ key_result: "635件", jan: "", feb: "", mar: "", apr: "", may: "", jun: "" }, { key_result: "160件", jan: "", feb: "", mar: "", apr: "", may: "", jun: "" }, { key_result: "6社/150%", jan: "", feb: "", mar: "", apr: "", may: "", jun: "" }];
const MOCK_MARKETING_DATA: MarketingRecord[] = [{ type: 'Source', label: 'Web (Inbound)', val1: 2500, val2: 400, val3: 80, val4: 5000000, val5: 0 }, { type: 'Source', label: 'Event', val1: 1200, val2: 150, val3: 30, val4: 8000000, val5: 0 }, { type: 'Campaign', label: '[EV] HR Momentum', val1: 99, val2: 46, val3: 72, val4: 0, val5: 0 }, { type: 'Funnel_Month', label: 'リード獲得', val1: 200, val2: 400, val3: 600, val4: 0, val5: 0 }, { type: 'Funnel_Month', label: '商談化', val1: 80, val2: 150, val3: 220, val4: 0, val5: 0 }];
const MOCK_PIPELINE_AGG: PipelineRecord[] = [{ type: 'Amount', segment: 'Enterprise', m1: 16289417, m2: 11874840, m3: 11849760, m4: 50826379, m5: 10098414, m6: 18688880 }, { type: 'Amount', segment: 'Mid', m1: 15735600, m2: 8430892, m3: 3949990, m4: 16900000, m5: 1621000, m6: 1924000 }, { type: 'Amount', segment: 'Small', m1: 2948800, m2: 1493000, m3: 902700, m4: 1229569, m5: 970200, m6: 803000 }];
const MOCK_OTHER_DATA: OtherRecord[] = [{ type: 'Segment', name: '企業代理店', val1: 1000, val2: 0, val3: 1100 }, { type: 'Partner', name: 'Partner A', val1: 1200, val2: 0, val3: 0 }];
const MOCK_ADVISOR_DATA: AdvisorRecord[] = [{ source: 'エスプール', cost: 500000, referrals: 10, lost: 4, ongoing: 4, won: 2, revenue: 3000000 }, { source: 'DYM', cost: 300000, referrals: 5, lost: 2, ongoing: 2, won: 1, revenue: 1500000 }];
...
export default function CBDashboard() {
  ...
  const fetchSheetData = async (key: keyof AppSettings) => {
    ...
    try {
      ...
      if (key === 'overview') {
        ...
        setNewSalesData(parseCSV(r1)); setExistingSalesData(parseCSV(r2)); setSalesListData(parseCSV(r3));
      } else if (key === 'other') {
        ...
        setAdvisorData(parseCSV(r1)); setNegListData(parseCSV(r2));
      } else if (key === 'pipeline') {
        const [r1, r2] = await Promise.all([fetch(baseUrl + 'Pipeline_Agg').then(r => r.text()), fetch(baseUrl + 'Lists').then(r => r.text())]);
        setPipelineData(parseCSV(r1)); setPipeListData(parseCSV(r2));
      } else if (key === 'okr') {
      ...
      alert('データ取得エラー: シート名が正しいか、権限があるか確認してください。');
    }
  };
  ...
  const renderContent = () => {
    switch (activeTab) {
      ...
      case 'sales': return <SalesAnalysisTab newData={newSalesData} existData={existingSalesData} listData={salesListData} />;
      case 'other': return <OtherSalesTab otherData={otherData} />;
      case 'marketing': return <MarketingAnalysisTab mktData={marketingData} />;
      case 'negotiation': return <NegotiationAnalysisTab advisorData={advisorData} listData={negListData} />;
      case 'pipeline': return <PipelineAnalysisTab pipeData={pipelineData} listData={pipeListData} />;
      ...
      <main className="flex-1 overflow-y-auto p-8 relative">
        <header className="flex justify-between items-center mb-8 border-b border-slate-200 pb-4">
          <h1 className="text-2xl font-bold text-slate-800">
            {activeTab === 'overview' ? '月次売上および今後の売上予測' : activeTab === 'sales' ? '企業直販売上分 (新規/既存)' : activeTab === 'other' ? 'その他売上分析 (代理店・優待・学校)' : activeTab === 'marketing' ? 'マーケ施策・分析' : activeTab === 'negotiation' ? '商談・トライアル分析レポート' : activeTab === 'pipeline' ? 'パイプライン分析' : activeTab === 'okr' ? 'OKR・今後のアクション' : 'データ連携設定'}
          </h1>
  ...
  return (
    <div className="space-y-6 max-w-5xl mx-auto animate-in fade-in">
      <div className="flex justify-between items-center mb-6">
        <div><h2 className="text-2xl font-bold flex items-center gap-2"><Database size={28} className="text-indigo-600"/> データ連携設定</h2><p className="text-sm text-slate-500 mt-1">各ページのGoogleスプレッドシートIDを登録してください。シート内のタブ名が一致している必要があります。</p></div>
        ...
        {Object.entries(settings).map(([key, config]: [string, any]) => (
          <div key={key} className="bg-white p-6 rounded-xl shadow-sm border border-slate-200 hover:border-indigo-300 transition-colors">
             <div className="flex justify-between items-start mb-4">
                <div><h3 className="font-bold text-lg text-slate-800 flex items-center gap-2">{config.name} {config.status === 'success' && <CheckCircle size={16} className="text-emerald-500"/>}</h3><p className="text-xs text-slate-500 mt-1">{config.description}</p><p className="text-xs text-indigo-400 mt-1">必要タブ: {config.tabs.join(', ')}</p></div>
...
const OverviewTab = ({ data }: { data: SalesRecord[] }) => {
  ...
  return (
     <div className="space-y-8 animate-in fade-in">
        ...
        <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-100 h-96"><h3 className="font-bold text-lg mb-4 text-slate-800">年間予実推移</h3><ResponsiveContainer width="100%" height="100%"><ComposedChart data={data} margin={{top: 20, right: 30, left: 20, bottom: 5}}><CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#f1f5f9" /><XAxis dataKey="month" /><YAxis /><Tooltip formatter={(v:any) => formatCurrency(v)} /><Legend /><Bar dataKey="sales_actual" name="実績" fill="#6366f1" barSize={30} /><Line type="monotone" dataKey="sales_budget" name="予算" stroke="#fbbf24" strokeWidth={3} /><Line type="monotone" dataKey="sales_forecast" name="見込" stroke="#10b981" strokeWidth={3} /></ComposedChart></ResponsiveContainer></div>
     </div>
  );
};
...
const SalesAnalysisTab = ({ newData, existData, listData }: { newData: NewSalesRecord[], existData: ExistingSalesRecord[], listData: ListRecord[] }) => {
  ...
  return (
    <div className="space-y-6 animate-in fade-in">
       <div className="flex gap-4 border-b border-slate-200 pb-1"><button onClick={()=>setSubTab('new')} className={`px-4 py-2 text-sm font-bold rounded-t-lg transition-colors ${subTab==='new' ? 'bg-indigo-600 text-white' : 'text-slate-500 hover:bg-slate-100'}`}>新規売上</button><button onClick={()=>setSubTab('existing')} className={`px-4 py-2 text-sm font-bold rounded-t-lg transition-colors ${subTab==='existing' ? 'bg-emerald-600 text-white' : 'text-slate-500 hover:bg-slate-100'}`}>既存売上</button></div>
       {subTab === 'new' ? (
   ...
   return (
      <div className="space-y-6 animate-in fade-in">
         <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <div className="bg-white p-6 rounded-xl shadow-sm h-80 border border-slate-100"><h3 className="font-bold mb-4 text-slate-700">セグメント別予実</h3><ResponsiveContainer width="100%" height="100%"><BarChart data={segments} layout="vertical" margin={{left: 20}}><CartesianGrid strokeDasharray="3 3" horizontal={false} /><XAxis type="number" /><YAxis dataKey="name" type="category" width={100} tick={{fontSize: 12}} /><Tooltip formatter={(v:any)=>formatCurrency(v)} /><Legend /><Bar dataKey="val1" name="予算" fill="#94a3b8" barSize={20} /><Bar dataKey="val3" name="実績" fill="#6366f1" barSize={20} /></BarChart></ResponsiveContainer></div>
...
const OkrActionTab = ({ okrData }: { okrData: OkrRecord[] }) => {
   ...
   return (
      <div className="space-y-8 animate-in fade-in">
         ...
         <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-100"><h3 className="font-bold mb-4 flex gap-2 text-slate-800"><Info size={20} className="text-indigo-600"/> 総括・コメント</h3><textarea className="w-full h-32 p-4 bg-slate-50 border rounded-lg text-sm resize-none" placeholder="コメント入力..."></textarea></div>
      </div>
   );
};