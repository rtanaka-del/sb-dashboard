...
const INITIAL_SETTINGS: AppSettings = {
  ...
  sales: { 
    id: '', 
    name: '企業直販売上分', 
    description: '必要なシート: New (新規KPI), Existing (既存KPI), Lists (カテゴリー: NewDeal, Renewed, NotRenewed, Fluctuation)', 
    status: 'idle' 
  },
  other: { 
    id: '', 
  ...
  negotiation: { 
    id: '', 
    name: '商談・トライアル分析', 
    description: '必要なシート: Advisor_CPA (顧問データ), Lists (カテゴリー: AdvisorDeal, Trial)', 
    status: 'idle' 
  },
  pipeline: { 
    id: '', 
    name: 'パイプライン分析', 
    description: '必要なシート: Pipeline_Agg (集計データ), Lists (カテゴリー: PipelineNew, PipelineExist)', 
    status: 'idle' 
  },
  okr: { 
    id: '', 
...
const parseCSV = (csvText: string): any[] => {
  ...
  return lines.slice(1).map(line => {
    ...
    headers.forEach((header, index) => {
      ...
      if (val) val = val.replace(/,/g, ''); // カンマ除去（数値変換用）
      
      if (val === '' || val === undefined || val === '-') {
        record[header] = null;
      } else if (
        !isNaN(Number(val)) && 
        // 以下のカラム名は数値変換せず文字列として扱う
...
// ==========================================
// 4. 共通UIコンポーネント (Common Components)
// ==========================================

// 円形プログレスバー (Segment Cardで使用)
...
// セグメント別カード (Enterprise, Mid, Small)
...
export default function CBDashboard() {
  ...
  // 日付管理（現在は固定）
  const [currentMonthIndex, setCurrentMonthIndex] = useState<number>(5); // 9月

  // 初期化処理：LocalStorageから設定を読み込む
  useEffect(() => {
  ...
  const saveSettings = () => {
    ...
    alert('設定を保存しました。次回アクセス時もこの設定が使用されます。');
  };
    ...
    try {
      ...
      if (key === 'overview') {
        ...
        const [r1, r2, r3] = await Promise.all([
          ...
          fetch(baseUrl + 'Lists').then(r => r.text()) // DealList
        ]);
      ...
      alert('データの取得に失敗しました。シート名や権限を確認してください。');
    }
  };

  // 左サイドバーからの「一括同期」ボタン用（デモ用として残していますが、設定タブからの個別同期を推奨）
  const handleGlobalSync = async () => {
    if(!sheetInput) return;
    setIsSyncing(true);
    // ここでは便宜上、OverviewのIDとして扱う
    updateSetting('overview', sheetInput);
    await fetchSheetData('overview');
    setIsSyncing(false);
  };
  ...
  const renderContent = () => {
    switch (activeTab) {
      case 'settings': 
        return <SettingsTab settings={settings} updateSetting={updateSetting} saveSettings={saveSettings} fetchSheetData={fetchSheetData} />;
      case 'overview': 
        return <OverviewTab data={salesData} />;
      case 'sales': 
        return <SalesAnalysisTab newData={newSalesData} existData={existingSalesData} listData={salesListData} />;
      case 'other': 
        return <OtherSalesTab otherData={otherData} />;
      case 'marketing': 
        return <MarketingAnalysisTab mktData={marketingData} />;
      case 'negotiation': 
        return <NegotiationAnalysisTab advisorData={advisorData} listData={negListData} />;
      case 'pipeline': 
        return <PipelineAnalysisTab pipeData={pipelineData} listData={pipeListData} />;
      case 'okr': 
  ...
  return (
    <div className="flex h-screen bg-slate-50 text-slate-800 font-sans overflow-hidden">
      ...
      <main className="flex-1 overflow-y-auto p-8 relative">
        {/* Header inside main content for better scroll handling */}
  ...
  return (
    <div className="space-y-6 max-w-5xl mx-auto">
      <div className="flex justify-between items-center mb-6">
        <div>
           <h2 className="text-2xl font-bold flex items-center gap-2"><Database size={28} className="text-indigo-600"/> データ連携設定</h2>
           <p className="text-sm text-slate-500 mt-1">各ダッシュボード画面に対応するGoogleスプレッドシートのIDを登録してください。</p>
        </div>
        <button 
                ...
                <div className="flex items-center gap-3">
                   ...
                   <button 
                      ...
                      className={`text-xs px-4 py-2 rounded-lg font-bold flex items-center gap-2 transition-all ${
                          !config.id ? 'bg-slate-100 text-slate-400' : 'bg-indigo-50 text-indigo-600 hover:bg-indigo-100 border border-indigo-200'
                      }`}
                   >
...
const OverviewTab = ({ data }: { data: SalesRecord[] }) => {
  if (!data || data.length === 0) {
      return (
          <div className="flex flex-col items-center justify-center h-96 text-slate-400 bg-slate-50 rounded-xl border border-dashed border-slate-300">
        ...
        <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-100">
           ...
           <table className="w-full text-right text-sm border-collapse">
              <thead className="bg-slate-50 text-slate-600">
                  <tr><th className="p-3 text-left">項目</th><th className="p-3">予算</th><th className="p-3">目標</th><th className="p-3">見込 (Forecast)</th><th className="p-3 text-indigo-600">対予算比</th></tr>
              </thead>
              <tbody>
                 {tableData.map((r, i) => (
                    <tr key={i} className="border-b border-slate-100 hover:bg-slate-50/50">
                       ...
                       <td className="p-3 font-bold text-indigo-600">{formatPercent(r.result/r.budget*100)}</td>
                    </tr>
                 ))}
              </tbody>
           </table>
        ...
        <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-100 h-96">
           ...
           <ResponsiveContainer width="100%" height="100%">
              <ComposedChart data={data} margin={{top: 20, right: 30, left: 20, bottom: 5}}>
                 ...
                 <Tooltip contentStyle={{ borderRadius: '8px', border: 'none', boxShadow: '0 4px 6px -1px rgb(0 0 0 / 0.1)' }} formatter={(v:any) => formatCurrency(v)} />
    ...
    <div className="space-y-6 animate-in fade-in">
       <div className="flex gap-4 border-b border-slate-200 pb-1">
          <button 
              ...
              className={`px-4 py-2 text-sm font-bold rounded-t-lg transition-colors ${subTab==='new' ? 'bg-indigo-600 text-white' : 'text-slate-500 hover:bg-slate-100'}`}
          >
              新規売上
          </button>
          <button 
              onClick={()=>setSubTab('existing')} 
              className={`px-4 py-2 text-sm font-bold rounded-t-lg transition-colors ${subTab==='existing' ? 'bg-emerald-600 text-white' : 'text-slate-500 hover:bg-slate-100'}`}
          >
              既存売上
          </button>
       </div>
          ...
          <div className="space-y-6">
             ...
             <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-100">
                ...
                <div className="overflow-x-auto">
                    ...
                    <thead className="bg-slate-50 text-slate-600">
                        <tr><th className="p-3">日付</th><th>顧客名</th><th>セグメント</th><th>金額</th><th>担当</th><th>メモ</th></tr>
                    </thead>
                    <tbody>
          ...
          <div className="space-y-6">
             ...
             <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-100">
                ...
                <div className="overflow-x-auto">
                    <table className="w-full text-sm text-left border-collapse">
                    <thead className="bg-slate-50 text-slate-600">
                        <tr><th className="p-3">種別</th><th>顧客名</th><th>金額</th><th>メモ</th></tr>
                    </thead>
                    <tbody>
         ...
         <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-100">
            ...
            <div className="overflow-x-auto">
                <table className="w-full text-sm text-right border-collapse">
                    <thead className="bg-slate-50 text-slate-600">
                        <tr><th className="p-3 text-left">ソース</th><th>リード</th><th>商談</th><th>受注</th><th>コスト</th><th>CPA (Cost/Lead)</th></tr>
                    </thead>
                    <tbody>
                        {sourceData.map((d,i)=>(
                            <tr key={i} className="border-b border-slate-100 hover:bg-slate-50">
                                ...
                                <td className="font-bold text-indigo-600">¥{(d.val4/d.val1).toFixed(0).toLocaleString()}</td>
                            </tr>
                        ))}
                    </tbody>
                </table>
         ...
         <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-100">
               ...
               <table className="w-full text-sm border-collapse">
                   <thead className="bg-slate-50 text-slate-600">
                       <tr><th className="p-2 text-left">キャンペーン名</th><th>Ent</th><th>Mid</th><th>Sml</th></tr>
                   </thead>
                   <tbody>
      ...
      <div className="space-y-8 animate-in fade-in">
         ...
         <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-100">
            ...
            <input 
                ...
                className="w-full border p-2 rounded mb-4 text-sm bg-slate-50" 
                placeholder="Google Drive IDを入力 (例: 1UijNv...)"
            />
            <div className="h-[500px] bg-slate-100 flex justify-center items-center rounded border border-slate-200">
               {url ? (
                   <iframe 
                   ...
                   <p className="text-slate-400">PDFレポートを表示するにはIDを入力してください</p>
               )}
            </div>
         </div>
            ...
            <div className="overflow-x-auto">
                <table className="w-full text-sm text-right border-collapse">
                    <thead className="bg-emerald-50 text-emerald-900">
                        <tr><th className="p-3 text-left">ソース</th><th>コスト</th><th>紹介</th><th>受注</th><th>売上</th></tr>
                    </thead>
                    <tbody>
             ...
             <div className="overflow-x-auto">
                 <table className="w-full text-sm text-left border-collapse">
                     <thead className="bg-amber-50 text-amber-900">
                         <tr><th className="p-3">取引先</th><th>セグメント</th><th>金額</th><th>開始日</th><th>メモ</th></tr>
                     </thead>
                     <tbody>
                         {trialDeals.map((d,i)=>(
                             <tr key={i} className="border-b border-slate-100 hover:bg-slate-50">
                                 ...
                                 <td>{d.amount?.toLocaleString()}</td>
                                 <td>{d.date}</td>
                                 <td className="text-xs text-slate-500 max-w-md">{d.extra1 || d.memo}</td>
                             </tr>
                         ))}
                     </tbody>
                 </table>
      ...
      <div className="space-y-8 animate-in fade-in">
             ...
             {['Enterprise','Mid','Small','Total'].map((t,i)=>(
                 <div key={i} className="bg-white p-4 rounded-xl shadow-sm border border-slate-200 text-center">
                     ...
                     <div className="relative w-full h-24">
                         <ResponsiveContainer>
                             <PieChart>
                                 <Pie data={[{value:85},{value:15}]} innerRadius={25} outerRadius={40} startAngle={180} endAngle={0} dataKey="value">
         ...
         <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-100">
            ...
            <div className="overflow-x-auto">
                <table className="w-full text-sm text-right border-collapse">
                    <thead className="bg-slate-50 text-slate-600">
                        <tr><th className="p-3 text-left">セグメント</th><th>1月</th><th>2月</th><th>3月</th><th>4月</th><th>5月</th><th>6月</th></tr>
                    </thead>
                    <tbody>
                        {amounts.map((d,i)=>(
                            <tr key={i} className="border-b border-slate-100 hover:bg-slate-50">
         ...
         <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-100">
               ...
               <table className="w-full text-xs text-left">
                   <thead className="text-slate-500"><tr><th className="p-2">時期</th><th>顧客名</th><th className="text-right">金額</th><th>確度</th></tr></thead>
                   <tbody>
                       {newPipe.map((d,i)=>(
                           <tr key={i} className="border-b border-slate-100 hover:bg-slate-50">
...
const OkrActionTab = ({ okrData }: { okrData: OkrRecord[] }) => {
   ...
   return (
      <div className="space-y-8 animate-in fade-in">
         {/* Top Cards */}
         <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="bg-white p-6 rounded-xl border-t-4 border-blue-500 shadow-sm hover:shadow-md transition-shadow">
                <div className="text-center mb-4">
                    <Rocket className="text-blue-500 mx-auto mb-2" size={32}/>
                    <h3 className="font-bold text-lg text-slate-800">Objective 1</h3>
                    <p className="text-xs text-slate-500 mt-2">受注拡大の加速装置を構築</p>
                </div>
            </div>
            <div className="bg-white p-6 rounded-xl border-t-4 border-teal-500 shadow-sm hover:shadow-md transition-shadow">
                <div className="text-center mb-4">
                    <Settings className="text-teal-500 mx-auto mb-2" size={32}/>
                    <h3 className="font-bold text-lg text-slate-800">Objective 2</h3>
                    <p className="text-xs text-slate-500 mt-2">CS基盤の構築とKSF発見</p>
                </div>
            </div>
            <div className="bg-white p-6 rounded-xl border-t-4 border-green-600 shadow-sm hover:shadow-md transition-shadow">
                <div className="text-center mb-4">
                    <Volume2 className="text-green-600 mx-auto mb-2" size={32}/>
                    <h3 className="font-bold text-lg text-slate-800">Objective 3</h3>
                    <p className="text-xs text-slate-500 mt-2">認知度基盤の構築</p>
                </div>
            </div>
         </div>
            ...
            <div className="w-full overflow-x-auto">
               <table className="w-full text-sm text-left border-collapse min-w-[1200px]">
                  <thead className="bg-slate-50 sticky top-0 z-10 text-slate-600">
                      <tr>
                          <th className="p-3 border-b border-slate-200 sticky left-0 bg-slate-50 z-20 min-w-[200px]">Key Result</th>
                  ...
                  <tbody className="divide-y divide-slate-100">
                     {safeOkrData.map((r,i)=>(
                        <tr key={i} className="hover:bg-slate-50 group">
                           <td className="p-3 border-r border-slate-100 font-bold sticky left-0 bg-white shadow-[2px_0_5px_-2px_rgba(0,0,0,0.1)] group-hover:bg-slate-50">
                               {r.key_result}
                           </td>
                           <td className="p-2 border-r border-slate-100"><textarea className="w-full h-16 bg-transparent border border-slate-200 rounded p-2 text-xs resize-none focus:outline-none focus:border-indigo-400 focus:bg-white" defaultValue={r.jan} placeholder="-"/></td>
                           <td className="p-2 border-r border-slate-100"><textarea className="w-full h-16 bg-transparent border border-slate-200 rounded p-2 text-xs resize-none focus:outline-none focus:border-indigo-400 focus:bg-white" defaultValue={r.feb} placeholder="-"/></td>
                           <td className="p-2 border-r border-slate-100"><textarea className="w-full h-16 bg-transparent border border-slate-200 rounded p-2 text-xs resize-none focus:outline-none focus:border-indigo-400 focus:bg-white" defaultValue={r.mar} placeholder="-"/></td>
                           <td className="p-2 border-r border-slate-100"><textarea className="w-full h-16 bg-transparent border border-slate-200 rounded p-2 text-xs resize-none focus:outline-none focus:border-indigo-400 focus:bg-white" defaultValue={r.apr} placeholder="-"/></td>
                           <td className="p-2 border-r border-slate-100"><textarea className="w-full h-16 bg-transparent border border-slate-200 rounded p-2 text-xs resize-none focus:outline-none focus:border-indigo-400 focus:bg-white" defaultValue={r.may} placeholder="-"/></td>
                           <td className="p-2"><textarea className="w-full h-16 bg-transparent border border-slate-200 rounded p-2 text-xs resize-none focus:outline-none focus:border-indigo-400 focus:bg-white" defaultValue={r.jun} placeholder="-"/></td>
                        </tr>
                     ))}
                  </tbody>
               </table>
         ...
         <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-100">
             ...
             <textarea 
                 className="w-full h-32 p-4 bg-slate-50 border border-slate-200 rounded-lg text-sm focus:outline-none focus:border-indigo-400 resize-none placeholder-slate-400" 
